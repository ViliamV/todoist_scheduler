...
class API:
    ...
    def _verify_and_load_projects(self):
        projects = {}
        response = requests.get("https://api.todoist.com/rest/v1/projects", headers=self._headers(post=False))
    ...
    def create_task(self, content, project, due_date, priority=2):
        response = requests.post(
            ...
            data=json.dumps({"content": content, "project_id": project, "due_date": due_date, "priority": priority}),
    ...
    def create_project(self, name, parent=None):
        if name not in self.projects.keys():
            ...
            if parent:
                data["parent"] = parent
            response = requests.post(
                "https://api.todoist.com/rest/v1/projects", data=json.dumps(data), headers=self._headers()
            )
    ...
    @classmethod
    def create_new(cls, root, missing_conf=False, missing_location=False, missing_token=False):
        import pickle
        import toml
        ...
        if missing_conf:
            print("This is Boost for Todoist.")
            print('Creating default configuration file "config"')
        ...
        if missing_token or missing_location or missing_conf:
            print("API Token")
            print("Go to https://todoist.com/Users/viewPrefs?page=integrations and copy API token")
