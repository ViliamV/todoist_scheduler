...
while add_new_task:
    ...
    for i, proj in enumerate(projects):
        print("{} - {}".format(i, proj))
    project_number = int(
        input("To which project would you like to add a task? (enter a number): ")
    )
    ...
    early = input(
        "How early should be task added to Todoist? (input can be such as '3 days' or '1 W'): "
    )
    ...
    if task_type in "rR":
        interval = input(
            "What should be the repetition interval? (input can be such as '1 day', '2 W' or '1 month on the last day'): "
        )
    priorities = {
        0: "no priority",
        1: "low priority",
